...
const STRINGS = {
  en: {
    tagline: "Vacant rooms, straight from the landlord. No middlemen, no fake numbers.",
  ...
  sw: {
    tagline: "Vyumba wazi, moja kwa moja kutoka kwa mwenye nyumba. Hakuna madalali, hakuna namba za uongo.",
...
function filterListings(listings, criteria = {}) {
  const { location, maxPrice, amenities = [], roomType, favoritesOnly, favorites = [], near, coords, radius = 5 } = criteria;
  let list = listings
...
function Footer({ t }) {
  return (
    <div style={{ maxWidth: 720, margin: "24px auto 0", padding: "16px", borderTop: `1px solid ${COLORS.border}` }}>
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <img
        ...
        style={{ width: "100%", height: 180, objectFit: "cover", borderRadius: 4, display: "block", cursor: "zoom-in" }}
      />
      {photos.length > 1 ? (
...
function ListingCard({ listing, rightAction, rotateSeed, distance, onReport, isFavorite, onToggleFavorite, onShowQR, landlordBio, trackView, isOwnerView }) {
  ...
  return (
    ...
    >
      ...
      {listing.room_type ? (
        ...
        >
          {ROOM_TYPES.find((r) => r.key === listing.room_type)?.label || listing.room_type}
        </span>
      ) : null}
...
export default function App() {
  ...
  const addSlot = async () => {
    if (!newSlotLabel.trim() || !slotManagerTarget) return;
    setSlotSaving(true);
    const nextSlots = [...(slotManagerTarget.viewing_slots || []), { id: uid(), label: newSlotLabel.trim(), taken: false }];
  ...
  const confirmClaim = async () => {
    ...
    // Fire-and-forget: if SMS isn't set up or fails, the interest is still
    // saved and the hunter's experience isn't affected.
    fetch("/api/send-sms", {
      method: "POST",
                ...
                {rooms.map((r, index) => (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                      <div style={{ fontSize: 12, fontWeight: 800, color: COLORS.blue }}>Room {index + 1}</div>
                      {rooms.length > 1 ? (
                        <button
                          type="button"
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                      <TextField label="Room" value={r.roomLabel} onChange={(v) => updateRoom(index, { roomLabel: v })} placeholder="e.g. Bedsitter, 1st floor" />
                      <TextField label="Rent (KES / month)" type="number" value={r.price} onChange={(v) => updateRoom(index, { price: v })} placeholder="e.g. 12000" />
                      <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 12, color: COLORS.gray, fontWeight: 600 }}>
                        Room type
                        <select
                          value={r.roomType}
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: COLORS.card, borderRadius: 10, padding: 20, width: "100%", maxWidth: 380 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                placeholder="e.g. Sat 23 Aug, 10:00 AM"
                style={{ flex: 1, border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: "9px 10px", fontSize: 13 }}
              />
              <button
                onClick={addSlot}
                disabled={slotSaving || !newSlotLabel.trim()}
                style={{
                  background: COLORS.blue,
                  color: "#fff",
                  border: "none",
