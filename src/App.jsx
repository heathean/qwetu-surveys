...
const MY_INTERESTS_KEY = "jjk_my_interests_v1";
const MY_VIEWINGS_KEY = "jjk_my_viewings_v1";
...
const AMENITIES = [
  { key: "water", label: "Reliable water", icon: Droplet, categories: ["room", "house"] },
  { key: "balcony", label: "Balcony", icon: Wind, categories: ["room", "house"] },
  { key: "parking", label: "Parking", icon: Car, categories: ["room", "house", "warehouse"] },
  { key: "wifi", label: "Wifi ready", icon: Wifi, categories: ["room", "house"] },
  { key: "furnished", label: "Furnished", icon: Sofa, categories: ["room", "house"] },
  { key: "security", label: "Gated / security", icon: ShieldCheck, categories: ["room", "house", "warehouse"] },
  { key: "loading_dock", label: "Loading dock", icon: Truck, categories: ["warehouse"] },
  { key: "three_phase", label: "Three-phase power", icon: Zap, categories: ["warehouse"] },
  { key: "cctv", label: "CCTV", icon: Video, categories: ["room", "house", "warehouse"] },
];

function amenitiesForCategory(category) {
...
// A short, pleasant two-note "pop" — synthesized so no audio file is
// needed. Fails silently if the browser blocks audio before any
// user interaction, which is normal and not worth surfacing as an error.
function playNotificationPop() {
  try {
    ...
    // Audio isn't available — the visual notification still shows fine.
  }
}
...
function SearchAssistant({ open, onClose, listings, favorites, onApply }) {
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text: "Hi! Tell me what you're looking for — a room, house, or warehouse. Try \"bedsitter in Kilimani under 15k\" or \"warehouse with three-phase power\". I can also help you compare listings or find viewing times — just ask.",
    },
  ]);
  ...
  const sendText = (text) => {
    ...
    setTimeout(() => {
      ...
      onApply(criteria);
      const reply = { role: "assistant", text: summarize(results) };
      const newMessages = [reply];

      // If a category came through and they haven't mentioned any
      // amenities yet, offer quick, category-specific follow-up chips —
      // e.g. "three-phase power" for a warehouse, not general questions.
      if (criteria.category && !criteria.amenities) {
        const options = amenitiesForCategory(criteria.category);
        if (options.length > 0) {
          newMessages.push({
            role: "assistant",
            text: `Need any of these for your ${CATEGORIES.find((c) => c.key === criteria.category)?.label.toLowerCase() || criteria.category}?`,
            quickReplies: options.slice(0, 4).map((a) => ({ label: a.label, text: `with ${a.label.toLowerCase()}` })),
          });
        }
      }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const savedInterests = localStorage.getItem(MY_INTERESTS_KEY);
    if (savedInterests) {
      try {
        setMyInterests(JSON.parse(savedInterests));
      } catch (e) {
        // ignore corrupt local interest tracking
      }
    }

    const savedViewings = localStorage.getItem(MY_VIEWINGS_KEY);
    if (savedViewings) {
      try {
        setMyViewings(JSON.parse(savedViewings));
      } catch (e) {
        // ignore corrupt local viewing bookings
      }
    }
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "interests" }, (payload) => {
        ...
        if (listing && listing.landlord_id === user.id) {
          ...
          setInterestNotifications((prev) => [note, ...prev]);
          setUnreadNotifCount((prev) => prev + 1);
          setToastNotification(note);
          setTimeout(() => {
            setToastNotification((current) => (current && current.id === note.id ? null : current));
          }, 5000);
        }
      })
      .subscribe();
  ...
  // Runs once per sign-in only — NOT on every profile.phone keystroke,
  // which used to reload all listings on every character typed (flicker).
  const claimedForUserRef = React.useRef(null);
  useEffect(() => {
    if (user && profile.phone && claimedForUserRef.current !== user.id) {
  ...
  const confirmPostListing = async () => {
    setSaving(true);
  ...
  const confirmClaim = async () => {
    ...
    const { data: newInterestId, error: interestErr } = await supabase.rpc("register_interest", {
    ...
    // Remember locally so this browser can "unmark" the interest later,
    // and so they can't accidentally mark the same room twice.
    setMyInterests((prev) => {
      const next = { ...prev, [claimTarget.id]: newInterestId };
    ...
    if (chosenSlot) {
      setMyViewings((prev) => {
        const next = [
          ...prev.filter((v) => v.listingId !== claimTarget.id),
          {
            id: uid(),
            listingId: claimTarget.id,
            buildingName: claimTarget.building_name,
            roomLabel: claimTarget.room_label,
            label: chosenSlot.label,
            datetime: chosenSlot.datetime,
          },
        ];
        localStorage.setItem(MY_VIEWINGS_KEY, JSON.stringify(next));
        return next;
      });
    }
  ...
  const handleUnmarkInterest = async (listingId) => {
    const interestId = myInterests[listingId];
    if (!interestId) return;
    await supabase.rpc("remove_interest", { p_interest_id: interestId, p_listing_id: listingId });
    setMyInterests((prev) => {
      const next = { ...prev };
    ...
    setMyViewings((prev) => {
      const next = prev.filter((v) => v.listingId !== listingId);
      localStorage.setItem(MY_VIEWINGS_KEY, JSON.stringify(next));
      return next;
    });
    loadListings();
    ...
    <div style={{ background: COLORS.paper, minHeight: "100vh", fontFamily: "sans-serif", color: COLORS.ink }}>
      ...
      {toastNotification ? (
        ...
        >
          <Bell size={16} />
          <span>
            {toastNotification.hunterName} is interested in your {toastNotification.roomLabel} at {toastNotification.buildingName}!
          </span>
          <button
            onClick={() => setToastNotification(null)}
            style={{ background: "none", border: "none", color: COLORS.blueDark, cursor: "pointer" }}
            aria-label="Dismiss"
          >
            <X size={15} />
          </button>
      ...
      {showNotifBell ? (
        ...
        >
          ...
          >
            <div style={{ fontWeight: 800, fontSize: 13, marginBottom: 10, display: "flex", alignItems: "center", gap: 5 }}>
              <Bell size={14} /> Notifications
            </div>
            {interestNotifications.length === 0 ? (
              <div style={{ fontSize: 12, color: COLORS.gray }}>No interest in the last hour.</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {interestNotifications.map((n) => (
                  ...
                  >
                    <span style={{ flex: 1 }}>
                      <strong>{n.hunterName}</strong> is interested in {n.roomLabel} at {n.buildingName}
                      <span style={{ display: "block", color: COLORS.gray, fontSize: 11, marginTop: 2 }}>{minutesAgo(n.createdAt)}</span>
                    </span>
                    <button
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "20px 20px 40px" }}>
        ...
        {loading ? (
          ...
          <div>
            {myViewings.filter((v) => new Date(v.datetime).getTime() > Date.now()).length > 0 ? (
              <div
                style={{
                  background: COLORS.greenBg,
                  border: `1px solid ${COLORS.green}`,
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 6, fontWeight: 700, color: COLORS.green, fontSize: 12, marginBottom: 8 }}>
                  <CalendarClock size={15} /> Your upcoming viewings
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {myViewings
                    .filter((v) => new Date(v.datetime).getTime() > Date.now())
                    .sort((a, b) => new Date(a.datetime) - new Date(b.datetime))
                    .map((v) => (
                      <div key={v.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
                        <span style={{ flex: 1 }}>
                          {v.buildingName} ({v.roomLabel}): {v.label}
                        </span>
                        <span style={{ color: COLORS.mustard, fontWeight: 800, fontSize: 12 }}>{timeUntil(v.datetime)}</span>
                        <button
                          onClick={() => {
                            setMyViewings((prev) => {
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
              {amenitiesForCategory(filterCategory).map((a) => {
                const Icon = a.icon;
                ...
                {vacantListings.map((l, i) => (
                  <ListingCard
                    ...
                    rightAction={
                      myInterests[l.id] ? (
                        <button
                          onClick={() => handleUnmarkInterest(l.id)}
                          style={{
                            display: "flex",
                            alignItems: "center",
                            gap: 4,
                        ...
                        >
                          <Check size={12} /> Interested — tap to remove
                        </button>
                      ) : (
                        <button
                          onClick={() => openClaim(l)}
          ...
          <div>
            ...
            >
              <div
                onClick={(e) => e.stopPropagation()}
                style={{ width: "100%", maxWidth: 420, maxHeight: "85vh", overflowY: "auto", position: "relative" }}
              >
                <button
                  onClick={() => setShowProfileModal(false)}
                  style={{
              ...
              {editingProfile ? (
                <div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                    <div style={{ fontWeight: 800, fontSize: 14 }}>Set up your profile</div>
                    <button
                  ...
                  >
                        ...
                        <button
                          ...
                          onClick={() =>
                            updateRoom(index, {
                              category: c.key,
                              roomType: "",
                              amenities: r.amenities.filter((k) => amenitiesForCategory(c.key).some((a) => a.key === k)),
                            })
                          }
                          style={{
                            fontSize: 12,
                    ...
                    <div style={{ marginTop: 12 }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {amenitiesForCategory(r.category).map((a) => {
                          const Icon = a.icon;
      ...
      {compareIds.length >= 2 ? (
        ...
        >
          <Scale size={16} /> Compare ({compareIds.length}{compareIds.length < COMPARE_MAX ? `/${COMPARE_MAX}` : ""})
        </button>
      ) : null}
      ...
      {showPostConfirm ? (
        ...
        >
          <div
            onClick={(e) => e.stopPropagation()}
            style={{ background: COLORS.card, borderRadius: 10, padding: 20, width: "100%", maxWidth: 380 }}
          >
            <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 10 }}>Confirm before posting</div>
            <div style={{ fontSize: 13, color: COLORS.ink, marginBottom: 14 }}>
              You're about to post at <strong>{buildingInfo.buildingName}</strong>, {buildingInfo.location}:
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
              {rooms.map((r, i) => (
                ...
                >
                  <span>
                    <strong>{CATEGORIES.find((c) => c.key === r.category)?.label}</strong> — {r.roomLabel}
                  </span>
                  <span style={{ color: COLORS.blue, fontWeight: 700 }}>KES {Number(r.price || 0).toLocaleString()}</span>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
